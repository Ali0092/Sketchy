 ...
 *
 * Every named shape is run through [sketch] once, right where it's built, before it's stroked,
 * filled or folded into a hole — so the hand-inked wobble carries through to every mode (a filled
 * silhouette's edge is exactly the sketched line, not a clean one underneath it). A shape that gets
 * merged with another via [Path.addPath] (a multi-segment accent, a hole made of more than one
 * piece) is sketched *before* merging — see [sketch]'s doc for why the merged result can't be.
 *
...
internal fun DrawScope.drawHomeIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    ...
    val body = sketch(roundedPolygonPath(listOf(12f to 3f, 20f to 10.5f, 20f to 21f, 4f to 21f, 4f to 10.5f), r))
    val door = sketch(roundRectPath(10f, 14.5f, 4f, 6.5f, cornerRadius(style, 1.2f)))
    when (style) {
...
internal fun DrawScope.drawSearchIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val ring = sketch(ellipsePath(10.5f, 10.5f, 6.5f, 6.5f))
    val handle = sketch(
        Path().apply {
    ...
    when (style) {
        IconStyle.Filled -> {
            val innerHole = sketch(ellipsePath(10.5f, 10.5f, 4.2f, 4.2f))
...
internal fun DrawScope.drawSettingsIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    ...
    val gear = sketch(roundedPolygonPath(points, r))
    val hub = sketch(ellipsePath(cx, cy, 3.2f, 3.2f))
    when (style) {
...
internal fun DrawScope.drawUserIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val head = sketch(ellipsePath(12f, 8f, 4f, 4f))
    val shoulders = sketch(
        Path().apply {
            moveTo(d(4.5f), d(21f))
...
internal fun DrawScope.drawMailIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 2f)
    val body = sketch(roundRectPath(3f, 5f, 18f, 14f, r))
    val flap = sketch(
        Path().apply {
            moveTo(d(3.6f), d(6f))
            lineTo(d(12f), d(13f))
...
internal fun DrawScope.drawCalendarIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 2f)
    val body = sketch(roundRectPath(3.5f, 4.5f, 17f, 16f, r))
    val headerLine = sketch(
        Path().apply {
...
internal fun DrawScope.drawClockIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val face = sketch(ellipsePath(12f, 12f, 9f, 9f))
    val hourHand = sketch(Path().apply { moveTo(d(12f), d(12f)); lineTo(d(12f), d(7.5f)) })
    val minuteHand = sketch(Path().apply { moveTo(d(12f), d(12f)); lineTo(d(16f), d(13.5f)) })
...
internal fun DrawScope.drawEditIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    ...
    if (style != IconStyle.Filled) {
        drawPath(
            sketch(Path().apply { moveTo(d(6.2f), d(17.8f)); lineTo(d(14.6f), d(9.4f)) }),
            color = tint,
...
internal fun DrawScope.drawTrashIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 1.5f)
    val bin = sketch(roundedPolygonPath(listOf(6f to 8f, 18f to 8f, 16.5f to 21f, 7.5f to 21f), r))
    val lines = Path().apply {
        listOf(10f, 12f, 14f).forEach { x ->
            addPath(sketch(Path().apply { moveTo(d(x), d(11f)); lineTo(d(x), d(18f)) }))
        }
    }
...
internal fun DrawScope.drawCameraIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 2f)
    val body = sketch(roundRectPath(3f, 7f, 18f, 12.5f, r))
    val lens = sketch(ellipsePath(12f, 13f, 3.6f, 3.6f))
    val viewfinder = sketch(roundRectPath(9f, 4.5f, 6f, 3f, cornerRadius(style, 1f)))
    when (style) {
        IconStyle.Filled -> {
...
internal fun DrawScope.drawImageIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 2f)
    val frame = sketch(roundRectPath(3f, 4f, 18f, 16f, r))
    val sun = sketch(ellipsePath(8.2f, 9f, 1.8f, 1.8f))
    val mountain = sketch(
        Path().apply {
...
internal fun DrawScope.drawLockIcon(style: IconStyle, tint: Color, strokeWidth: Float) {
    val r = cornerRadius(style, 2f)
    val body = sketch(roundRectPath(4f, 10.5f, 16f, 11f, r))
    val shackle = sketch(
        Path().apply {
            moveTo(d(7f), d(10.5f))
            lineTo(d(7f), d(8f))
