 ...
 *  silhouette needs no internal cut-out. */
internal fun DrawScope.paintIcon(path: Path, style: IconStyle, tint: Color, strokeWidth: Float) {
    when (style) {
        IconStyle.Filled -> drawPath(path, color = tint, style = Fill)
        IconStyle.TwoTone -> {
            drawPath(path, color = tint.copy(alpha = tint.alpha * 0.28f), style = Fill)
            drawPath(path, color = tint, style = iconStroke(IconStyle.Default, strokeWidth))
        }
        else -> drawPath(path, color = tint, style = iconStroke(style, strokeWidth))
    }
}
