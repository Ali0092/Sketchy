...
internal fun DrawScope.drawOrderFavoritesScene(t: Float, colors: SketchyColors) {
    ...
    if (popScale > 0.02f) {
        ...
        withTransform({ scale(scaleX = popScale, scaleY = popScale, pivot = bubblePivot) }) {
            sketchCircle(bubblePivot, 14f, colors.accent, width = 2.4f)
            sketchLine(pt(206f, 124f), pt(206f, 136f), colors.accent, 2.2f)
            sketchLine(pt(200f, 130f), pt(212f, 130f), colors.accent, 2.2f)
        }
    }
    ...
    for (i in 0..1) {
        ...
        sketchLine(
            ...
            colors.inkSoft.copy(alpha = 1f - steamPhase), 1.6f
        )
    }

    twinkle(64f, 100f, 3f, t, 0.5f, colors.inkSoft)
...
internal fun DrawScope.drawFastDeliveryScene(t: Float, colors: SketchyColors) {
    ...
    drawPath(route, color = colors.inkFaint, style = dashed())
    val pin = Path().apply {
        moveTo(d(260f), d(90f))
    ...
    sketchLine(pt(40f + drift, scooterY - 10f), pt(70f + drift, scooterY - 10f), colors.inkSoft, 1.8f)
    sketchLine(pt(30f - drift, scooterY + 2f), pt(64f - drift, scooterY + 2f), colors.inkSoft, 1.8f)

    groundHint(roadY + 24f, colors.inkFaint)
}
