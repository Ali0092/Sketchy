...
internal fun DrawScope.drawTrackSpendingScene(t: Float) {
    // head + hair
    sketchCircle(pt(122f, 104f), 21f, width = 2.4f)
    val hair = Path().apply {
        moveTo(d(104f), d(96f))
    ...
    stroke(body)
    sketchLine(pt(96f, 220f), pt(90f, 260f))
    sketchLine(pt(148f, 220f), pt(154f, 260f))
    sketchLine(pt(84f, 260f), pt(100f, 260f))
    sketchLine(pt(146f, 260f), pt(162f, 260f))
    ...
    sketchCircle(pt(252f, coinY.coerceAtMost(236f)), 9f, Accent.copy(alpha = coinAlpha), width = 2.2f)
    sketchLine(pt(248f, 216f), pt(256f, 216f), AccentTeal.copy(alpha = 0.5f), 2f)

    twinkle(60f, 90f, 3f, t, 0.3f, InkSoft)
    twinkle(220f, 76f, 3f, t, 0.6f, Accent)
    groundHint(276f)
}
...
internal fun DrawScope.drawGrowSavingsScene(t: Float) {
    ...
    drawPath(line, color = InkFaint, style = thin(1.6f))
    ...
    stroke(piggy, Ink, 2.4f)
    sketchLine(pt(150f, 218f), pt(166f, 214f), Ink, 2.4f)
    sketchCircle(pt(120f, 236f), 2f, filled = true)
    sketchLine(pt(112f, 262f), pt(116f, 274f))
    sketchLine(pt(176f, 266f), pt(180f, 278f))
    ...
    withTransform({ rotate(degrees = sway, pivot = leafPivot) }) {
        ...
        positions.forEach { (lx, ly) ->
            sketchCircle(pt(lx, ly), 12f, AccentTeal, width = 2.2f)
            sketchLine(pt(lx - 4f, ly), pt(lx + 4f, ly), Accent, 1.6f)
            sketchLine(pt(lx, ly - 4f), pt(lx, ly + 4f), Accent, 1.6f)
        }
    }

    // person watering the tree
    sketchCircle(pt(224f, 150f), 19f, width = 2.4f)
    val faceSmile = Path().apply {
        moveTo(d(220f), d(157f))
        quadraticTo(d(223f), d(160f), d(226f), d(157f))
    }
    stroke(faceSmile, Ink, 1.6f)
    sketchLine(pt(224f, 169f), pt(224f, 176f))
    ...
    stroke(pBody)
    sketchLine(pt(202f, 244f), pt(198f, 270f))
    sketchLine(pt(246f, 244f), pt(250f, 270f))
    ...
    if (streamOn) {
        sketchLine(pt(188f, 202f), pt(174f, 210f), AccentTeal.copy(alpha = 0.6f), 1.6f)
        sketchLine(pt(184f, 208f), pt(170f, 216f), AccentTeal.copy(alpha = 0.5f), 1.4f)
    }

    twinkle(70f, 90f, 3f, t, 0.4f, Accent)
    twinkle(250f, 100f, 3f, t, 0.7f, InkSoft)
    groundHint(284f)
}
